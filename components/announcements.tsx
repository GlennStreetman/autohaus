...
function Announcements(p: props) {
    ...
    return (
        <div className='flex flex-col'>
            ...
            <div className='mb-2 subpixel-antialiased font-body text-lg w-3/5 mx-auto flex '>{p?.bannerTexts?.link ? <NextLinkButton text={p.bannerTexts.linkText} link={p.bannerTexts.link} icon={<></>} /> : ''}</div>
        </div>
    );
}
