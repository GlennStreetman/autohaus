...
function MapFAQ(p: props) {
    ...
    // @ts-ignore
    const mapResultLimit = p?.faq
        ? Object.keys(Array.from({ length: Object.keys(p.faq).length })).map((key) => {
              return (
    ...
    return (
        <div>
            {FAQContainer}
            {edit !== false ? <EditFAQ faq={edit} setEdit={setEdit} setFAQ={p.setFAQ} getFAQ={p.getFAQ} setShowAdd={p.setShowAdd} /> : <></>}
        </div>
    );
}
